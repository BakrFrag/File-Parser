...
class TestxlsxParser(unittest.TestCase):
    ...
    def test_with_file_not_exist(self):
        ...
        """
        xlsx_parser = XlsxParser(self.valid_customers,
                                 self.xlsx_vehicles_not_exist)
        function_result = xlsx_parser.read_xlsx(
            xlsx_parser.vehicles_file, kind="vehicle")
        self.assertEqual(function_result['error'], "file {} not exists".format(
            self.xlsx_vehicles_not_exist))

        xlsx_parser = XlsxParser(
            self.xlsx_customers_not_exist, self.valid_vehicles)
        function_result = xlsx_parser.read_xlsx(
            xlsx_parser.customers_file, kind="customer")
        self.assertEqual(function_result['error'], "file {} not exists".format(
            self.xlsx_customers_not_exist))
    ...
    def test_with_headers_miss_match(self):
        ...
        """
        xlsx_parser = XlsxParser(self.valid_customers,
                                 self.xlsx_vehicle_headers_miss_match)
        function_result = xlsx_parser.read_xlsx(
            xlsx_parser.vehicles_file, kind="vehicle")
        self.assertEqual(
        ...
        xlsx_parser = XlsxParser(
            self.xlsx_customer_headers_miss_match, self.valid_vehicles)
        function_result = xlsx_parser.read_xlsx(
            xlsx_parser.customers_file, kind="customer")
        self.assertEqual(
